...
{
	public class TableQueriesFactory<T> where T : class
	{
		...
		/*
		public TableDelegates<T>.RemoveDuplicatesFunc RemoveDuplicatesFunc { get; protected set; }
		*/

		public TableQueriesData<T> Create()
		{
			TableQueriesData<T> queries = new TableQueriesData<T>();
		...
		public TableQueriesData<T, KeyType> Create<KeyType>()
		{
			if (KeyProperties.Length != 1) {
				string errorMsg = typeof(T).Name + " requires a single key";
				throw new InvalidOperationException(errorMsg);
			}
			TableQueriesData<T, KeyType> queries = new TableQueriesData<T, KeyType>();
		...
		/// </summary>
		private IEnumerable<object> GetTempSetParams(string[] columnNames, IHasDefaultAttribute[] defaultValues)
		{
		...
		/// [x] = [x], [y] = getdate()
		/// </summary>
		private IEnumerable<object> GetSetParams(string[] columnNames, PropertyInfo[] properties, IHasDefaultAttribute[] defaultValues)
		{
			if (columnNames.Length == 0)
				return new List<object>() { "" };
			List<object> result = new List<object>();
		...
		/// @a,@b,@c
		/// </summary>
		private IEnumerable<object> GetValues(PropertyInfo[] properties, IHasDefaultAttribute[] defaultValues)
		{
			if (properties.Length == 0)
				return new List<object>() { "" };
			List<object> result = new List<object>();
